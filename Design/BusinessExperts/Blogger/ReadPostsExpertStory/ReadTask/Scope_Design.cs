...
public class Scope_Design : Design<Scope> {
    ...
    public async void ItCan_PopulateResponseWithPosts() {
        ...
        await solution.Mock.Received().Read(Arg.Any<Request>(), Arg.Any<CancellationToken>());
    }
...
public class SolutionMockBuilder {
    ...
    public SolutionMockBuilder ProvidesPosts() {
        Results =
        [
            new Post(1, "Title1", "Content1", DateTime.UtcNow),
            new Post(2, "Title2", "Content2", DateTime.UtcNow),
            new Post(3, "Title3", "Content3", DateTime.UtcNow)
        ];
        Mock.Read(default, default).ReturnsForAnyArgs(Results);
        return this;
    }
}
