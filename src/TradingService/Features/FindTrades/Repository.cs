...
namespace TradingService.Features.FindTrades;

internal class RepositoryAdapter(IRepository repository) : IRepositoryAdapter {
    public async Task<List<Trade>> Find(FindTradesRequest request, CancellationToken token) {
    ...
    private DataModel.TradeSide? ToDataModelTradeSide(string? side) => string.IsNullOrWhiteSpace(side) ? null : ToDataModelTradeSide(Enum.Parse<TradeSide>(side, true));

    private DataModel.TradeSide? ToDataModelTradeSide(TradeSide? side) => side switch {
        null => null,
        TradeSide.Buy => DataModel.TradeSide.Buy,
        TradeSide.Sell => DataModel.TradeSide.Sell,
        _ => throw new ArgumentOutOfRangeException(nameof(side), side, null)
    };

    private TradeSide ToDomainTradeSide(DataModel.TradeSide side) => side switch {
        DataModel.TradeSide.Buy => TradeSide.Buy,
        DataModel.TradeSide.Sell => TradeSide.Sell,
        _ => throw new ArgumentOutOfRangeException(nameof(side), side, null)
    };

    private Trade MakeItEntityFrameworkFree(DataModel.Trade dataModel) => new(
